...
public class NamePartRevision extends Persistable {
     ...
     * @param requestedBy the user that proposed the revision. Null if the revision was generated by an automated process.
     ...
     * @param mnemonicEqClass the representative of the equivalence class the mnemonic belongs to. This is used to ensure uniqueness of mnemonics on certain levels when treating similar looking names (for example, containing 0 vs. O, 1 vs. l) as equal.
     */
    ...
    /**
     *  @return The representative of the equivalence class the mnemonic belongs to. This is used to ensure uniqueness of mnemonics on certain level when treating similar looking names (for example, containing 0 vs. O, 1 vs. l) as equal.
     */
    ...
    /**
     *  @return The user who processed the revision in the request / approve workflow. Null if still pending or if the revision  @return was processed by an automated process.
     */
    public @Nullable UserAccount getProcessedBy() { return processedBy; }

    /**
     *  @return The comment the user gave when processing the revision in the request / approve workflow. Null if still pending  @return or if no comment was given.
     */
    public @Nullable String getProcessorComment() { return processorComment; }
    
    /**
     * Updates the revision's status in the request / approve workflow as either approved, rejected or canceled. This completes the workflow and no further status changes are possible after that.
     *
